...
import { Euro, Info, FileText, CheckCircle, AlertCircle, ArrowRight, Download, Users, Loader2 } from 'lucide-react';
...
export default function InscriptionsPage() {
  ...
  useEffect(() => {
    document.title = "Tarifs & Inscriptions | US Créteil Badminton";
    fetch('/api/admin/inscriptions-config')
      .then(r => r.json())
  ...
  if (isLoading) return (
    <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#040817]">
      <Loader2 className="animate-spin text-[#0065FF]" size={48} />
    </div>
  );

  const tarifs = config?.tarifs || [];
  const notes = config?.notes || [];
  const saison = config?.saison || 'Saison 2024 - 2025';
  const inscriptionLink = config?.inscriptionLink || 'https://myffbad.fr';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-8">
            ...
            <div className="flex flex-col gap-4">
              {tarifs.map((tarif, idx) => (
                <div key={idx} className="bg-white dark:bg-[#0f172a] p-5 lg:p-6 rounded-[1.5rem] border-2 border-slate-100 dark:border-white/5 shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:-translate-y-1 group">
                  <div>
                  ...
                  <div
                    className="px-6 py-3 rounded-xl font-black text-2xl italic tracking-tighter shrink-0 shadow-lg shadow-black/10"
                    style={{
                      backgroundColor: tarif.badgeColor,
                      color: tarif.textDark ? '#081031' : '#ffffff'
                    }}
                  >
                    {tarif.price}
                  </div>
            ...
            {notes.length > 0 && (
              <div className="bg-orange-50 dark:bg-orange-500/10 border-2 border-orange-200 dark:border-orange-500/20 rounded-[2rem] p-6 lg:p-8 mt-10">
                ...
                <ul className="space-y-4">
                  {notes.map((note, idx) => (
                    <li key={idx} className={`flex items-start gap-3 ${idx === notes.length - 1 && notes.length > 1 ? 'bg-white/50 dark:bg-black/20 p-4 rounded-xl border border-orange-200/50 dark:border-orange-500/10' : ''}`}>
                      {idx === notes.length - 1 && notes.length > 1
                      ...
                      <p className="text-sm font-bold text-orange-800 dark:text-orange-200 leading-relaxed">{note}</p>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-32 space-y-6">
            ...
            <div className="bg-white dark:bg-[#0f172a] rounded-[2rem] p-8 lg:p-10 border border-slate-200 dark:border-white/5 shadow-xl">
              ...
              <div className="mt-10">
                <a href={inscriptionLink} target="_blank" rel="noopener noreferrer" className="w-full bg-[#0065FF] hover:bg-[#0052cc] text-white py-4 px-6 rounded-xl font-[900] uppercase text-sm tracking-widest flex items-center justify-center gap-3 transition-all shadow-[0_0_15px_rgba(0,101,255,0.4)] group">
