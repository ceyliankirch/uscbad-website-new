...
import { ChevronLeft, ChevronRight, Trophy, Calendar, MapPin, ExternalLink, Activity, X, Loader2 } from 'lucide-react';

const team1Players = [
  { id: 1, name: "Romain Credou", role: "Simple Homme / Double", rank: "N2", img: "/assets/romain-credou.png", gender: 'male', stats: { played: 14, wins: 11, style: "Offensif" }, description: "Joueur explosif avec un smash surpuissant. Spécialiste du simple, il apporte sa puissance en double." },
  { id: 2, name: "Manon Espen", role: "Simple Dame / Mixte", rank: "N2", img: "/assets/manon-espen.png", gender: 'female', stats: { played: 16, wins: 13, style: "Contre-attaque" }, description: "Véritable muraille sur le court. Sa couverture de terrain est exceptionnelle et use ses adversaires." },
  { id: 3, name: "Vincent Espen", role: "Double Homme / Mixte", rank: "N1", img: "/assets/vincent-espen.png", gender: 'male', stats: { played: 12, wins: 9, style: "Créateur / Filet" }, description: "Meneur de jeu redoutable. Sa vision sur la zone avant est un atout majeur pour dicter l'échange." },
  { id: 4, name: "S. Golubickaite", role: "Double Dame", rank: "N1", img: "/assets/samantha-golubickaite.png", gender: 'female', stats: { played: 10, wins: 8, style: "Polyvalente" }, description: "Excellente puncheuse en fond de court, elle s'adapte à tous les rythmes tactiques imposés." },
  { id: 5, name: "Yohann Osvald", role: "Simple Homme", rank: "N2", img: "/assets/yohann-osvald.png", gender: 'male', stats: { played: 8, wins: 6, style: "Endurance" }, description: "Capable de tenir des rallyes interminables pour faire plier physiquement ses adversaires." },
  { id: 6, name: "Ulyana Volskaya", role: "Simple Dame", rank: "N1", img: "/assets/ulyana-volskaya.png", gender: 'female', stats: { played: 15, wins: 14, style: "Technique" }, description: "Précision diabolique. Ses amortis croisés et son toucher de volant surprennent souvent." },
  { id: 7, name: "Nolhan Rihouey", role: "Double / Mixte", rank: "N2", img: "/assets/nolhan-rihouey.png", gender: 'male', stats: { played: 11, wins: 7, style: "Agressif" }, description: "Spécialiste de la pression constante. Il ne laisse aucun répit et joue toujours vers l'avant." },
];

const fallbackRankingN1 = [
  { rank: 1, name: "BC Chambly Oise", pts: 41, played: 8 },
  { rank: 2, name: "US Créteil (Fallback)", pts: 38, played: 8, isUs: true },
...
const fallbackNextMatchesN1 = [
  { date: "24 MARS", time: "16:00", home: "US CRÉTEIL", away: "FLUME ILLE", location: "Gymnase Casalis", isHome: true },
  { date: "14 AVRIL", time: "15:00", home: "VGA STELLA", away: "US CRÉTEIL", location: "Stade Chéron", isHome: false },
  { date: "05 MAI", time: "16:00", home: "US CRÉTEIL", away: "RACING CLUB", location: "Gymnase Casalis", isHome: true },
];

// FONCTION INTELLIGENTE : Coupe le nom après 2 mots pour le mettre sur 2 lignes
...
export default function InterclubsPage() {
  ...
  // ==========================================
  // STATES DES CLASSEMENTS DYNAMIQUES
  // ==========================================
  ...
  useEffect(() => {
    const fetchIcbadData = async () => {
      try {
        ...
        // On interroge notre propre API qui fait le pont avec ICBAD
        ...
        if (result.success && result.data) {
          ...
          console.log("🔍 INFO SCRAPER ICBAD :", result.debug);
        }
      } catch (err) {
        console.error("Erreur lors de la synchro avec l'API locale ICBAD", err);
      } finally {
        setIsScraping(false);
      }
    };
  ...
  return (
    <div className="bg-white dark:bg-[#040817] min-h-screen font-['Montserrat'] text-[#081031] dark:text-white transition-colors duration-300">
        ...
        <div className="relative z-10 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 flex flex-col justify-center h-full">
          ...
          {selectedPlayer ? (
            <div className="w-full flex flex-col lg:flex-row items-center justify-between animate-in fade-in zoom-in duration-500 relative pt-12 lg:pt-0">
              <button onClick={() => setSelectedPlayer(null)} className="absolute top-0 right-0 lg:-top-12 lg:right-4 z-50 text-white/50 hover:text-white flex items-center gap-2 font-bold uppercase tracking-widest text-xs transition-colors cursor-pointer bg-white/5 lg:bg-transparent px-4 py-2 rounded-full lg:p-0">
                <X size={20} /> Fermer
              </button>
              ...
              <div className="w-full lg:w-[50%] aspect-square max-h-[500px] lg:max-h-[700px] relative z-10 flex items-center justify-center order-1 lg:order-2 shrink-0">
                ...
                <img src={selectedPlayer.img} alt={selectedPlayer.name} className="relative z-20 w-full h-full object-contain object-bottom drop-shadow-[0_20px_50px_rgba(0,0,0,0.6)]" />
              </div>
              ...
              <div className="w-full lg:w-[50%] flex flex-col justify-center pt-8 lg:pt-0 z-20 order-2 lg:order-1 text-center lg:text-left">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 lg:gap-8 mb-8 lg:mb-10 border-t border-white/10 pt-8">
                  <div className="flex flex-col"><span className="text-3xl lg:text-4xl font-[900] text-white italic">{selectedPlayer.stats.played}</span><span className="text-[9px] lg:text-[10px] text-white/50 uppercase tracking-widest font-bold mt-2">Matchs Joués</span></div>
                  <div className="flex flex-col"><span className="text-3xl lg:text-4xl font-[900] text-white italic">{selectedPlayer.stats.wins}</span><span className="text-[9px] lg:text-[10px] text-white/50 uppercase tracking-widest font-bold mt-2">Victoires</span></div>
                  <div className="flex flex-col"><span className="text-3xl lg:text-4xl font-[900] text-[#0EE2E2] italic">{Math.round((selectedPlayer.stats.wins / selectedPlayer.stats.played) * 100)}%</span><span className="text-[9px] lg:text-[10px] text-[#0EE2E2]/60 uppercase tracking-widest font-bold mt-2">Win Rate</span></div>
                  <div className="flex flex-col justify-center"><span className="text-sm lg:text-lg font-[900] text-white italic uppercase leading-none">{selectedPlayer.stats.style}</span><span className="text-[9px] lg:text-[10px] text-white/50 uppercase tracking-widest font-bold mt-2">Style de jeu</span></div>
                </div>
                <p className="text-white/60 font-bold text-xs lg:text-sm max-w-lg leading-relaxed border-l-0 lg:border-l-4 border-t-4 lg:border-t-0 pt-4 lg:pt-0 mx-auto lg:mx-0 border-[#0065FF] lg:pl-5">{selectedPlayer.description}</p>
              </div>
            </div>
          ) : (
            <div className="w-full flex flex-col items-center animate-in fade-in duration-500 mt-6 lg:mt-0">
              <div className="text-center mb-6 lg:mb-10">
                <h1 className="text-5xl md:text-7xl lg:text-8xl font-[900] uppercase italic tracking-tighter text-white drop-shadow-md">INTER<span className="text-[#0065FF]">CLUBS</span></h1>
                <p className="text-xs sm:text-sm font-bold text-[#0EE2E2] uppercase tracking-widest mt-2">Saison 2025-2026</p>
              </div>
              <div className="text-center mb-10 lg:mb-16">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#0EE2E2] text-[#081031] font-[900] uppercase text-[10px] tracking-widest italic mb-3"><Trophy size={14} /> Équipe Première</div>
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-[900] uppercase italic tracking-tighter text-white leading-none">NATIONALE <span className="text-[#0EE2E2]">1</span></h2>
              </div>
              <div className="relative w-full max-w-[400px] sm:max-w-[500px] lg:max-w-[600px] aspect-square flex justify-center items-center perspective-[1500px]" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
                {team1Players.map((player, index) => (
                  <div key={player.id} className="absolute w-[320px] sm:w-[450px] lg:w-[550px] aspect-square transition-all duration-700 ease-out cursor-pointer border-none" style={getCardStyle(index)} onMouseEnter={() => handleMouseEnter(index)} onMouseLeave={handleMouseLeaveCard} onClick={() => { if (activeIndex === index) setSelectedPlayer(player); }}>
                    <div className="w-full h-full relative transition-transform duration-150 ease-out flex flex-col justify-end items-center group pointer-events-none" style={{ transform: activeIndex === index ? `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` : 'rotateX(0deg) rotateY(0deg)' }}>
                      ...
                      <img src={player.img} alt={player.name} className="relative z-10 w-full h-full object-contain object-bottom drop-shadow-[0_15px_30px_rgba(0,0,0,0.5)] transition-transform duration-700 group-hover:scale-105" />
                    </div>
                    <div className={`absolute -bottom-10 lg:-bottom-12 left-1/2 -translate-x-1/2 z-20 transition-all duration-500 pointer-events-none ${activeIndex === index ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                      <div className="bg-white text-[#081031] px-6 sm:px-8 py-3 rounded-full font-[900] uppercase text-[9px] sm:text-[10px] lg:text-[12px] tracking-widest shadow-[0_10px_20px_rgba(0,0,0,0.4)] whitespace-nowrap">Découvrir le joueur</div>
                    </div>
                    <div className="absolute inset-0 z-30" onMouseMove={(e) => handleMouseMove(e, index)}></div>
                  </div>
                ))}
              </div>
              <div className="flex items-center gap-6 mt-16 sm:mt-20 lg:mt-24 z-20">
                <button onClick={handlePrev} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-[#0EE2E2] hover:text-[#081031] transition-all backdrop-blur-md"><ChevronLeft size={20} className="sm:w-5 sm:h-5" /></button>
                <div className="flex gap-2">{team1Players.map((_, idx) => (<div key={idx} className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ${activeIndex === idx ? 'w-6 sm:w-8 bg-[#0EE2E2]' : 'w-1.5 sm:w-2 bg-white/30'}`} />))}</div>
                <button onClick={handleNext} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-[#0EE2E2] hover:text-[#081031] transition-all backdrop-blur-md"><ChevronRight size={20} className="sm:w-5 sm:h-5" /></button>
              </div>
            </div>
          )}
        </div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
              ...
              <div className="flex flex-col">
                {liveRanking.slice(0, 6).map((team, idx) => (
              ...
              {liveMatches.slice(0, 3).map((match, idx) => (
                <div key={idx} className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex flex-col sm:flex-row overflow-hidden group hover:border-[#0EE2E2] transition-colors">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-center">
                    ...
                    {/* NOUVELLE DISPOSITION : Grille 3 colonnes pour alignement parfait avec TAILLE ORIGINALE */}
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-3 mb-3 w-full">
                      ...
                      <div className="text-slate-300 dark:text-slate-600 font-black italic text-sm shrink-0 flex justify-center px-1">
                        VS
                      </div>
      ...
      {/* =========================================================
          SECTION 3 : ÉQUIPES RÉGIONALES (DYNAMIQUES)
          ========================================================= */}
              ...
              <div className="p-6 lg:p-8 pl-10 flex-1">
                <div className="flex justify-between items-start mb-5">
                  <div>
                    ...
                    <div className="inline-block mt-1 bg-[#0EE2E2]/20 text-[#081031] dark:text-[#0EE2E2] px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Pré-National</div>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  {liveRankingEq2.slice(0, 4).map((team, idx) => (
                    <div key={idx} className={`flex justify-between items-center font-bold text-xs uppercase px-2 py-1.5 rounded-lg transition-colors ${team.isUs ? 'font-[900] italic text-[#081031] dark:text-white bg-[#0EE2E2]/10' : 'text-slate-500 dark:text-slate-400'}`}>
                      <span className="truncate pr-2">{team.rank}. {team.name}</span>
                      <span className={team.isUs ? 'text-[#0065FF] dark:text-[#0EE2E2]' : ''}>{team.pts}</span>
                    </div>
                  ))}
                </div>
              </div>
              <a href="https://icbad.ffbad.org/equipe/67636" target="_blank" rel="noopener noreferrer" className="block bg-[#081031] text-white text-center py-4 font-[900] uppercase text-[9px] tracking-widest hover:bg-[#0EE2E2] hover:text-[#081031] transition-colors mt-auto">
                Voir le calendrier complet <ExternalLink size={10} className="inline mb-0.5" />
              </a>
            </div>
            ...
            <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm relative overflow-hidden group flex flex-col">
              ...
              <div className="p-6 lg:p-8 pl-10 flex-1">
                ...
                <div className="space-y-2.5">
                  ...
                  {liveRankingEq3.slice(0, 4).map((team, idx) => (
                    <div key={idx} className={`flex justify-between items-center font-bold text-xs uppercase px-2 py-1.5 rounded-lg transition-colors ${team.isUs ? 'font-[900] italic text-[#081031] dark:text-white bg-[#0065FF]/10' : 'text-slate-500 dark:text-slate-400'}`}>
                      <span className="truncate pr-2">{team.rank}. {team.name}</span>
                      <span className={team.isUs ? 'text-[#0065FF]' : ''}>{team.pts}</span>
                    </div>
                  ))}
                </div>
              </div>
              <a href="https://icbad.ffbad.org/equipe/67684" target="_blank" rel="noopener noreferrer" className="block bg-[#081031] text-white text-center py-4 font-[900] uppercase text-[9px] tracking-widest hover:bg-[#0065FF] transition-colors mt-auto">
                Voir le calendrier complet <ExternalLink size={10} className="inline mb-0.5" />
              </a>
            </div>
      ...
      <section className="py-16 lg:py-24 border-t border-slate-200 dark:border-white/5">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 text-center">
          ...
          <a 
            href="https://icbad.ffbad.org/instance/USC94" 
            target="_blank" 
